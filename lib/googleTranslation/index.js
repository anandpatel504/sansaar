...
let folderPath = path.resolve('curriculum/test');
...
class CoursesSeeder {
  constructor(googleTranslate, courseName) {
    this.courseDir = path.resolve(`curriculum/${courseName}/PARSED_CONTENT/PROPERTIES_FILES`);
  ...
  async main() {
    folderPath = this.courseDir;
    const propertiesFiles = fs.readdirSync(folderPath);
    _.map(propertiesFiles, async (property) => {
      let fileData = JSON.parse(fs.readFileSync(`${folderPath}/${property}`));
      console.log(fileData, 'ppprty\n');

      let target = 'hi';
      for (var index of _.keys(fileData)) {
        // console.log(index, "index\n\n");
        let backtickStr;
        let splitedData = fileData[index];
        //   console.log(splitedData, "splitedDataaa\n");
        //   if (splitedData[1] !== undefined) {
        if (splitedData.includes('`')) {
          backtickStr = splitedData.split('`');
          // console.log(backtickStr, "spliteddata");
        ...
        console.log(splitedData, 'splitedData[1]');
        if (splitedData.includes('`')) {
          backtickStr = splitedData.split('`');
          // console.log(backtickStr, "spliteddata");
          // console.log(splitedData, "spliteddata");
      ...
      // console.log(`${property.slice(0, -7)}${target}.json`, "ppp");

      fs.writeFileSync(
        path.resolve(`${folderPath}/${property.slice(0, -7)}${target}.json`),
  ...
  //       if (token.type === 'code' && token.lang === 'typing') {
  //         exercise.push({ type: 'markdown', value: markDownContent });
  //         markDownContent = '';
  //         // exercise.push({ type: token.lang, value: token.text.split('').join(',') });
  //         exercise.push({ type: token.lang, value: token.text.split('') });
  //       }

  //       if (token.type === 'code' && token.lang === 'trytyping') {
  //         exercise.push({ type: 'markdown', value: markDownContent });
  //         markDownContent = '';
  //         // exercise.push({ type: token.lang, value: token.text.split('').join(',') });
  //         exercise.push({ type: token.lang, value: token.text.split('') });
  ...
  //       if (token.type === 'code' && token.lang === 'practicetyping') {
  //         exercise.push({ type: 'markdown', value: markDownContent });
  //         markDownContent = '';
  //         const [keys, suggested_words] = token.text.split('\n');
  //         // exercise.push({ type: token.lang, value: keys.split('').join(','), suggested_words });
  //         exercise.push({ type: token.lang, value: keys.split(''), suggested_words });
  //       }
  //     });
...
if (!module.parent) {
  ...
  const seeder = new CoursesSeeder(googleTranslate, courseName);
